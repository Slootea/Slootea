...
export class EmailService {
  ...
  async sendAppointmentNotification(
    ...
    try {
      ...
      const subject = this.generateSubject(eventType, data);
      const html = this.generateHtmlBody(eventType, data);
      const text = this.generateTextBody(eventType, data);

      // Send the email
      return this.sendEmail(data.organizationId, data.clientEmail, subject, html, text);
    } catch (error) {
